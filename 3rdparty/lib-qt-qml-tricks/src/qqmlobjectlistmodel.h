...
class QQmlObjectListProxyModel : public QSortFilterProxyModel {
    Q_OBJECT
    Q_PROPERTY(int count READ count NOTIFY countChanged)
public:
    explicit QQmlObjectListProxyModel (QObject *parent = Q_NULLPTR)
        : QSortFilterProxyModel (parent) {
        connect(this, &QQmlObjectListProxyModel::rowsInserted,
                this, &QQmlObjectListProxyModel::countChanged);
        connect(this, &QQmlObjectListProxyModel::rowsRemoved,
                this, &QQmlObjectListProxyModel::countChanged);
    }

    void setSourceModel(QQmlObjectListModelBase *objectListModel) {
        QSortFilterProxyModel::setSourceModel(objectListModel);
    }

signals:
    void countChanged();

public slots: // QML slots implementation (QQmlObjectListModelBase)
    int size (void) const {
        return rowCount();
    }
    int count (void) const {
    ...
    int roleForName (const QByteArray & name) {
        return qobject_cast<QQmlObjectListModelBase *>(sourceModel())->roleForName(name);
    }
    void append (QObject * item) {
        qobject_cast<QQmlObjectListModelBase *>(sourceModel())->append(item);
    }
    void prepend (QObject * item) {
        qobject_cast<QQmlObjectListModelBase *>(sourceModel())->prepend(item);
    }
    void remove (QObject * item) {
        qobject_cast<QQmlObjectListModelBase *>(sourceModel())->remove(item);
    }
    bool contains (QObject * item) const {
        QQmlObjectListModelBase *objectListModel = qobject_cast<QQmlObjectListModelBase *>(sourceModel());
        bool sourceModelContains = objectListModel->contains(item);

        if (sourceModelContains) {
            return mapFromSource(sourceModel()->index(objectListModel->indexOf(item), 0)).isValid();
        }

        return false;
    }
    int indexOf (QObject * item) const {
        return mapFromSource(sourceModel()->index(qobject_cast<QQmlObjectListModelBase *>(sourceModel())->indexOf(item), 0)).row();
    }
    QObject * get (int idx) const {
        return qobject_cast<QQmlObjectListModelBase *>(sourceModel())->get(mapToSource(index(idx, 0)).row());
    }
    QObject * get (const QString & uid) const {
        return qobject_cast<QQmlObjectListModelBase *>(sourceModel())->get(uid);
    }
    QObject * getFirst (void) const {
        return get(0);
    }
    ...
    QVariantList toVarArray (void) const {
        QVariantList varArray;
        for (int i = 0; i < rowCount(); i++) {
            varArray.append(QVariant::fromValue(get(i)));
        }
        return varArray;
    }

private:
    // Hide default setSourceModel() implementation
    void setSourceModel(QAbstractItemModel *sourceModel) final {
        QSortFilterProxyModel::setSourceModel(sourceModel);
    }
};

#define QQMLOBJECTLISTPROXYMODEL_TYPENAME "QQmlObjectListModelProxy"
